...
const inputStyle = {
  ...
  padding: "12px 14px", color: "#f1f1f1",
  fontSize: "0.95rem", outline: "none"
};

const btn = (color = "#f97316") => ({
  width: "100%", background: color, border: "none",
  borderRadius: "10px", padding: "14px", color: "white",
  fontWeight: "700", fontSize: "1rem", cursor: "pointer",
  marginTop: "8px"
});

export default function Analyze({ t, language }) {
  ...
  const addDed = () => {
    if (dedName && dedAmount) {
      setDeductions([...deductions, {
        name: dedName, amount: parseFloat(dedAmount)
      }]);
      setDedName("");
      setDedAmount("");
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await api.analyze({
        employer, regularHours, overtimeHours,
        hourlyRate, state, deductions, language
      });
    ...
    <div>
      ...
      <div style={card}>
        ...
        >
          {file ? (
            <>
              ...
              <div style={{ color: "#f97316", fontWeight: "600" }}>{file.name}</div>
            ...
            <>
              ...
              <div style={{ color: "#d1d5db", fontWeight: "600" }}>{t.upload_tap}</div>
        ...
        {file && (
          <button onClick={handleExtract} disabled={extracting}
                  style={{ ...btn("#374151"), marginTop: "12px",
                           opacity: extracting ? 0.6 : 1 }}>
            {extracting ? t.reading : t.read_btn}
          </button>
        )}
      </div>
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom: "14px" }}>
          <label style={labelStyle}>{t.employer_label}</label>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr",
          ...
          <div>
            <label style={labelStyle}>{t.state_label}</label>
            <select style={inputStyle} value={state}
        ...
        {/* Deductions */}
        <div>
          <label style={labelStyle}>{t.deductions_label}</label>

          {deductions.map((d, i) => (
            <div key={i} style={{
              display: "flex", justifyContent: "space-between",
              alignItems: "center", background: "#1f2937",
          ...
          <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
            <input
              ...
              placeholder={t.ded_placeholder}
            />
            <input
              style={{ ...inputStyle, width: "90px", padding: "10px 12px",
                       fontSize: "0.85rem" }}
              type="number" value={dedAmount}
              onChange={e => setDedAmount(e.target.value)}
              placeholder={t.amount_placeholder} min="0"
            />
      ...
      <button onClick={handleAnalyze} disabled={loading}
        ...
        {loading ? t.analyzing : `🔍 ${t.step3}: ${t.analyze_btn}`}
      </button>
          ...
          <div style={{
            ...
            <div style={{ fontWeight: "700", fontSize: "1.05rem" }}>
              {result.has_violation
                ? `🚨 $${result.total_money_owed.toFixed(2)} ${t.violation_found}`
                : `✅ ${t.no_violation}`}
            </div>
          </div>
          ...
          {result.has_violation && (
            <div style={card}>
              ...
              <button onClick={handleLetter} disabled={letterLoading}
                ...
                {letterLoading ? t.letter_loading : `✉️ ${t.letter_btn}`}
              </button>
              {letter && (
                <div style={{
